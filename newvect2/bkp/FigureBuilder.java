...
public class FigureBuilder {
    ...
    void assignAngles(){
        int wl = w*2-1; int hl = h*2-1;
        figures.get(labels[0][0]-1).addAngle(new Point(0,0));
        figures.get(labels[w-1][0]-1).addAngle(new Point(wl,0));
        figures.get(labels[w-1][h-1]-1).addAngle(new Point(wl,hl));
        figures.get(labels[0][h-1]-1).addAngle(new Point(0,hl));
    }
    ...
    Figure collect_figure(Point start){
        ...
        do {
            ...
            ArrayList<Path> np = pc.getNearPaths(current);
            f.addBorders(np);
